     ...
     // 2. letter count, case-insensitive
        ...
        final Reducer red2 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                Cons a = value;
                while (a != null) {
                  init = Integer.decode((String) first((Cons)(first(a))));
                  sum = sum + init;
                  a = rest(a);
                }
                mr.collect_reduce(key, new Integer(sum));
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map3 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                while (st.hasMoreTokens()) {
                  mr.collect_map(value = st.nextToken(), list("1"));
                }
            } };
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                Cons val = value;
                while (val != null) {
                  init = Integer.decode((String) first((Cons)(first(val))));
                  lst = cons(init, lst);
                  val = rest(val);
                }
                mr.collect_reduce(key, nreverse(lst));
            } };
        ...
        final Mapper map5 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                StringTokenizer st = new StringTokenizer(value);
                while (st.hasMoreTokens()) {
                  mr.collect_map(value = st.nextToken(), list(key));
                }
            } };
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                String[] str = value.split(",");
                mr.collect_map(str[0], list(str[2]));
            } };
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                mr.collect_reduce(key, list(((double) sum / numRate), numRate));
            } };
